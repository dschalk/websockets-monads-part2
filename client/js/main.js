  ...
  return h('div',{style: style3}, 
   [  h('div',{style: { width: '60%', textAlign: 'left', marginLeft: 40, marginRight: '17%', fontSize: '20px'}}, 
    [ h('div', [
      ...
      h('p', 'HINTS: The best way to get off to a good start is to click ROLL and then score 1 point by making the number 20. Score = 0! Right away you have five points. You can use the same technique when you reach 5, 10, 15, and 20. Click ROLL when you are at twenty, then compute 20 to get a point, and you jump to 25. Goal! But you can\'t get to 25 from 24. Computing 20 jumps you all the way to 30. '  ),
...
h('p', 'Clicking numbers and operators calls updateNums and UpdateOps, respectively. They call updateCalc. updateCalc (below) clearly displays the flow of the application. First, mMZ2 and mMZ4 get locked, acquiring the code that might eventually get executed in their "p" attribute arrays. Then, in the third part of the tupple, tests are performed that might release the code being held in mMZ2 and mMZ4. "send" requests a new dice roll from the server. ' ),
   ...
   h('button', {on: { mouseenter: update4e, mouseleave: update4l, click: updatePauseDemo }, style: style4},
      ...
      h('p', 'MonadIter can be used to filter results while computations are in progress. For example, suppose I wanted to know if every integer x between 0 and 100 has a counterpart y between 0 and 10,000 such that the square root of x*x + y*7 is a whole number. 3 has 4 since the square root of 3*3 + 4*4 is 25, but what about 17, or 95? ' ),
